...
public class Board {
	...
	public Board(int size){
		...
		for (int i = 0 ; i<size_; i++) {
			for (int j = 0 ; j< size_; j++) {
				...
				gameBoard_[i][j] = ElementInBoard.EMPTY;
		...
		p = new Position(size_/2-1, size_/2-1);
		makeInColor(ElementInBoard.WHITE, p.getRow(),p.getColumn());
		p = new Position(size_/2, size_/2);
		makeInColor(ElementInBoard.WHITE, p.getRow(),p.getColumn());
		
		//initialize black squares
		p = new Position(size_/2 -1, size_/2);
		makeInColor(ElementInBoard.BLACK, p.getRow(),p.getColumn());
		p = new Position(size_/2, size_/2 - 1);
		makeInColor(ElementInBoard.BLACK, p.getRow(),p.getColumn());
	}
	...
	public boolean isCellWhite(int row, int col){
		...
		return getCell(pos) == ElementInBoard.WHITE;
	}

	public boolean isCellBlack (int row, int col){
		Position pos = new Position(row, col);
		return getCell(pos) == ElementInBoard.BLACK;
	}

	public boolean isCellEmpty(int row, int col){
		Position pos = new Position(row, col);
		return getCell(pos) == ElementInBoard.EMPTY;
	}
	...
	public void makeInColor(ElementInBoard c, int row, int col) {
		Position pos = new Position(row, col);
		gameBoard_[pos.getRow()][pos.getColumn()] = c;
		//remove from list of empty cells
		emptyCells_.remove(pos);
	}
	...
	public void flipColorInRange(Position prevStart, Position end, int rowJumps, int colJumps) {
		...
		while (curr != end) {
			...
			if (e == ElementInBoard.WHITE) {
				gameBoard_[curr.getRow()][curr.getColumn()] = ElementInBoard.BLACK;
			} else if (e == ElementInBoard.BLACK) {
				gameBoard_[curr.getRow()][curr.getColumn()] = ElementInBoard.WHITE;
			}
		}
	}
	...
	public boolean isInBoardRange(Position loc){
		...
		return (loc.getRow() >= 0 && loc.getColumn() >= 0 && loc.getRow() < size_ 
				&& loc.getColumn() < size_);
	}

	public boolean isEdge(Position loc){
		return (loc.getRow()==0 || loc.getRow()==(size_-1) 
				|| loc.getColumn()==0 || loc.getColumn()==(size_-1));
	}

	public boolean isBoardFull(){
		//if list is empty - board is full
		return emptyCells_.isEmpty();
	}


	public Iterator<Position> emptyCellsBegin(){
		return emptyCells_.listIterator(0);
	}

	public Iterator<Position> emptyCellsEnd(){
		return emptyCells_.listIterator(emptyCells_.size());
	}

	public ElementInBoard getCell(Position loc){
		return gameBoard_[loc.getRow()][loc.getColumn()];
	}
